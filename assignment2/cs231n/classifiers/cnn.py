...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    pad=(filter_size-1)/2
    stride=1
    H_out=(H+2*pad-filter_size)/stride+1
    W_out=(W+2*pad-filter_size)/stride+1
    ############################################################################
  ...
  def loss(self, X, y=None):
    ...
    loss+=0.5*self.reg*np.sum(grads['W3']**2)
    grads['W3']+=self.reg*W3
    dout,grads['W2'],grads['b2']=affine_relu_backward(dout,cache2)
    loss+=0.5*self.reg*np.sum(grads['W2']**2)
    grads['W2']+=self.reg*W2
    dout=dout.reshape(N,F,H_out,W_out)
    dout,grads['W1'],grads['b1']=conv_relu_pool_backward(dout,cache1)
    loss+=0.5*self.reg*np.sum(grads['W1']**2)
