...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    if self.use_batchnorm == False:   
        ...
        if self.use_dropout:    
            out,cache_dropout[0]=dropout_forward(out,self.dropout_param)
        for i in range (2,self.num_layers):
            out,cache[i-1]=affine_relu_forward(out,self.params['W'+str(i)],self.params['b'+str(i)])
            if self.use_dropout:    
    ...
    if self.use_batchnorm:
        cache={}
        out,cache[0]=affine_batchnorm_relu_forward(X,self.params['W1'],self.params['b1'],self.params['gamma1']
                                                   ,self.params['beta1'],self.bn_params[0])
        if self.use_dropout:    
            out,cache_dropout[0]=dropout_forward(out,self.dropout_param)
        for i in range (2,self.num_layers):
            out,cache[i-1]=affine_batchnorm_relu_forward(out,self.params['W'+str(i)],self.params['b'+str(i)],
    ...
    ############################################################################
    loss,grad=softmax_loss(scores,y)
    dx,grads['W'+str(self.num_layers)],grads['b'+str(self.num_layers)]=affine_backward(grad,cache_scores)
    ...
    if self.use_batchnorm:
        for i in range (self.num_layers-1,0,-1):
            ...
            dx,grads['W'+str(i)],grads['b'+str(i)],grads['gamma'+str(i)],grads['beta'+
                                                                               str(i)]=affine_batchnorm_relu_backward(dx,
                                                                                                                      cache[i-1])
